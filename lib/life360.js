...
const $log = require('log4js').getLogger('life360');
...
*/
exports.authenticate = function(username, password, phone, countryCode) {
  return new Promise((resolve, reject) => {
    ...
    $log.info(`Logging in as user: ${username}, phone: ${phone}`);
    request(options)
      .then(response => {
        ...
        $log.info(`Logged in as user: ${username}, phone: ${phone}, access_token: ${session.access_token}`);
        resolve(session);
      })
      .catch(err => {
...
*/
exports.circles = function(session) {
  ...
  return new Promise((resolve, reject) => {
    ...
    $log.info(`Fetching circles at ${LIFE360_CIRCLES_URL}`);
    request(options)
      .then(response => {
...
*/
exports.circle = function(session, circleId) {
  return new Promise((resolve, reject) => {
    if (!session) throw new Error("session not specified");
    ...
    $log.info(`Fetching circle at ${LIFE360_CIRCLE_URL}`);
    request(options)
      .then(response => {
